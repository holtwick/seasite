...
var SeaSite = exports.SeaSite = function () {
    function SeaSite(srcPath) {
        ...
        if (basePath == null) {
            ...
            // Filter files
            var files = (0, _fileutil.walkSync)(srcPath).filter(function (file) {
                return pathMatchesPatterns(file, opt.includePatterns) && !pathMatchesPatterns(file, opt.excludePatterns);
            });
    ...
    _createClass(SeaSite, [{
        ...
        value: function paths(pattern, exclude) {
            var urlPaths = (0, _fileutil.walkSync)(this.basePath).filter(function (file) {
                return pathMatchesPatterns(file, pattern) && !pathMatchesPatterns(file, exclude);
            });
            urlPaths.sort();
            return urlPaths;
        }
