...
var SeaSite = function () {
  ...
  (0, _createClass2["default"])(SeaSite, [{
    ...
    value: function handle(pattern, handler) {
      ...
      var _iterator4 = _createForOfIteratorHelper(urlPaths),
          _step4;
      ...
      try {
        var _loop = function _loop() {
          ...
          if (_this.exists(urlPath)) {
            content = _this.read(urlPath);
          }
            ...
            if (ret !== false) {
              ...
              if (ret.ignore !== true) {
                ...
                if ((0, _dom.isDOM)(_content)) {
                  _this.writeDOM(_content, p);
                } else if (_content) {
                  _this.write(p, _content);
                } else {
          ...
          if (ret && ret.then) {
            ret.then(solve)["catch"](function (err) {
              _log["default"].error('Promise error', err);
            });
          } else {
            solve(ret);
          }
