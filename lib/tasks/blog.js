...
function blog(site) {
    ...
    site.handle(opt.pattern, function (content, path) {
        var md = (0, _lib.parseMarkdown)(content);
        var props = md.props;
        var html = md.html;
    ...
    (0, _lib.setXMLMode)(false);

    var xml = (0, _lib.prependXMLIdentifier)(atomContent.xml());
    site.write('/blog/atom.xml', xml);
    site.write('/atom.xml', xml);

    // Blog Archive
    var $ = (0, _dom.dom)(opt.template(site));
    if ($) {
        var group = 'blog';
