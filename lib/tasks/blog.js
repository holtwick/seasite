...
function blog(site) {
    ...
    site.handle(opt.pattern, function (content, path) {
        ...
        var m = /(\d\d\d\d)-(\d\d)-(\d\d)/.exec(path);
    ...
    // Sort the posts
    entries = _lodash2.default.sortBy(entries, 'date').reverse();

    // RSS
    var atomContent = (0, _index.xml)((0, _jsx.jsx)(
        'rss',
        ...
        (0, _jsx.jsx)(
            ...
            (0, _jsx.jsx)(
                'pubDate',
                null,
                (0, _dateformat2.default)(maxDate || new Date(), 'isoDateTime')
            )
        )
    ));
